...
- **Exposure budget.** Opponents do not throw units away, so weigh each threat by its `return_fire.risk`. A `free` attack (return fire under half the attacker's wounds, no likely charge back) is one the opponent will take; `trade` is a real decision for them; `suicide` is usually bait. Every major unit in `summary.major_units_under_free_attack` needs an explicit purpose; more than one without a purpose is the headline finding. `summary.major_units_exposed` alone overstates the danger against long-ranged armies.
